...
struct JSON
{
    ...
    int type;                       /// The type of the item, see JSON Types.
	...
	JSON() : next(NULL), prev(NULL), child(NULL), type(0), valueint(0), valuefloat(0), currentdir(NULL)  { name = newstring(""); valuestring = newstring(""); }
