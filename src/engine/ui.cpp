...
namespace UI
{
    ...
    {
        glBegin(GL_TRIANGLE_STRIP);
        glTexCoord2f(tc[1].x, tc[1].y); glVertex2f(x+w, y);
        glTexCoord2f(tc[0].x, tc[0].y); glVertex2f(x,   y);
        glTexCoord2f(tc[2].x, tc[2].y); glVertex2f(x+w, y+h);
        glTexCoord2f(tc[3].x, tc[3].y); glVertex2f(x,   y+h);
        glEnd();
    }
    ...
    struct Gradient : FillColor
    {
        ...
        void draw(float sx, float sy)
        {
            ...
            Color::def();
            glBegin(GL_TRIANGLE_STRIP);
            glVertex2f(sx+w, sy);   (dir == HORIZONTAL ? color2 : color).attrib();
            glVertex2f(sx,   sy);   color.attrib();
            glVertex2f(sx+w, sy+h); color2.attrib();
            glVertex2f(sx,   sy+h); (dir == HORIZONTAL ? color : color2).attrib();
            glEnd();
			glEnable(GL_TEXTURE_2D);
            defaultshader->set();
            if(type==MODULATE) glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    ...
    struct Triangle : Shape
    {
        ...
        void draw(float sx, float sy)
        {
			...
			glDisable(GL_TEXTURE_2D);
            color.init();
            glBegin(type == OUTLINE ? GL_LINE_LOOP : GL_TRIANGLES);
            glVertex2fv(&vec2(sx, sy).add(a).x);
            glVertex2fv(&vec2(sx, sy).add(b).x);
            glVertex2fv(&vec2(sx, sy).add(c).x);
            glEnd();
            glColor3f(1, 1, 1);
			glEnable(GL_TEXTURE_2D);
            defaultshader->set();
        ...
        void draw(float sx, float sy)
        {
            ...
            if(type == OUTLINE)
            {
                ...
                for(int angle = 0; angle < 360; angle += 360/15)
                    glVertex2fv(&vec2(sincos360[angle]).mul(r).add(center).x);
                glEnd();
            }
            else
            {
                ...
                glVertex2fv(&center.x);
                glVertex2f(center.x + r, center.y);
