...
void savevslot(stream *f, int slot)
{
	...
	Slot &st = *vs.slot;
	char *name = newstring(st.sts[TEX_DIFFUSE].name);
	cutextension(name);
	f->printf("exec %s/%s.tex\n", texturedir, name);
}

// saves the textures of a map into stream f
// and merge out all unused ones of the texture-list
void saveusedvslots(stream *f)
{
...
void generatemapcfg(char *filename)
{
	if(noedit()) return;
	if(filename && filename[0]) cutextension(filename);
	defformatstring(fn) ("%s/%s.cfg", mapdir, filename ? filename : game::getclientmap());
	stream *f = openutf8file(path(fn), "w");
	if(!f) { conoutf("not able to write %s", fn); return; }
	f->putline("// This cfg was autogenerated");
	f->putline("texturereset");
	saveusedvslots(f);
	delete f;
}
ICOMMAND(generatemapcfg, "s", (char *s), generatemapcfg(s));
